...
describe("getAddressBalances", () => {
  const request = new Request("https://x/api/address/z1abc/balances");
  ...
  it("preserves a precision-critical 21-digit amount string untouched", async () => {
    ...
    const res = await getAddressBalances(request, env, ctx, { address: "z1abc" });
...
describe("getAddressTransactions", () => {
  function txReq(query: string): Request {
    return new Request(`https://x/api/address/z1abc/transactions${query}`, { method: "GET" });
  }

  it("passes through {data,pagination} from upstream", async () => {
    vi.mocked(nomIndexerFetch).mockResolvedValue({
      data: [{ hash: "h1", address: "z1abc" }],
      pagination: { page: 1, page_size: 50, total: 1 },
    });
    const res = await getAddressTransactions(txReq("?page=1&sort=desc&k=a"), env, ctx, { address: "z1abc" });
  ...
  it("synthesizes pagination for a bare array and preserves a big amount string", async () => {
    ...
    vi.mocked(nomIndexerFetch).mockResolvedValue([{ hash: "h2", address: "z1abc", amount: big }]);
    const res = await getAddressTransactions(txReq("?page=1&sort=desc&k=b"), env, ctx, { address: "z1abc" });
